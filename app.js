...
// ═══════════════════════════════════════════════════════
// NEXUS — app.js
// 100% live. No demo data. No fake auth. No coming soon.
// ═══════════════════════════════════════════════════════
...
async function handleLogin() {
  ...
  if (!data.user.email_confirmed_at) {
    showError(errEl, 'Please confirm your email first. Check your inbox.'); return;
  }

  const { data: profile, error: profileErr } = await supabase
    .from('profiles').select('*').eq('id', data.user.id).single();
  if (profileErr || !profile) { showError(errEl, 'Profile not found. Please sign up again.'); return; }

  currentUser = {
    id: data.user.id, name: profile.name, username: profile.username,
    email: data.user.email, vibe: profile.vibe, avatar: profile.avatar_url || null
  };
  localStorage.setItem('nexus_user', JSON.stringify(currentUser));
  showApp();
}
...
async function handleSignup() {
  ...
  if (!/^[a-z0-9_]{3,20}$/.test(username)) { showError(errEl, 'Username: 3-20 chars, letters/numbers/underscore only.'); return; }
  ...
  if (existing) { showError(errEl, `@${username} is already taken.`); return; }

  // Sign up — name/username/vibe go into user metadata so the DB trigger can read them
  ...
  if (needsConfirm) {
    // Supabase requires email confirmation — user must click the link
    document.getElementById('confirmEmail').textContent = email;
    showPanel('confirmPanel');
  } else {
    // Email confirmation disabled in Supabase — session is immediately active
    await supabase.from('profiles').upsert({ id: data.user.id, name, username, vibe });
...
function updateProfileUI() {
  ...
  if (avatar) {
    ...
    if (img) { img.src = avatar; img.style.display = 'block'; document.getElementById('profileFallback').style.display = 'none'; }
  }
  updateStats();
}
...
async function renderChatList() {
  ...
  [...(sentMsgs||[]), ...(receivedMsgs||[])].forEach(msg => {
    ...
    if (!pid) return;
    const ex = partnerMap.get(pid);
    if (!ex || new Date(msg.created_at) > new Date(ex.created_at)) partnerMap.set(pid, msg);
  });

  list.innerHTML = '';
  if (!partnerMap.size) {
    list.innerHTML = '<div class="empty-state">No conversations yet.<br>Tap <strong>+</strong> to find someone.</div>'; return;
  }

  const pids = Array.from(partnerMap.keys());
  const { data: profiles } = await supabase.from('profiles').select('id,name,username,vibe').in('id', pids);
  profiles?.sort((a,b) => new Date(partnerMap.get(b.id)?.created_at||0) - new Date(partnerMap.get(a.id)?.created_at||0));
  profiles?.forEach(p => {
    const latest  = partnerMap.get(p.id);
    const initial = p.name.charAt(0).toUpperCase();
    const div = document.createElement('div');
    div.className = 'chat-item';
    div.onclick = () => openChat({ id: p.id, name: p.name, username: p.username, vibe: p.vibe }, 'user');
    div.innerHTML = `
      <div class="chat-avatar"><div class="avatar-fallback grad-${charGrad(initial)}">${initial}</div></div>
      <div class="chat-info">
        <div class="chat-name">${escapeHtml(p.name)}</div>
        <div class="chat-preview">${escapeHtml(latest?.text||'')}</div>
      </div>
      <div class="chat-meta"><span class="chat-time">${timeAgo(latest?.created_at)}</span></div>
    `;
...
async function renderGroupList() {
  ...
  list.innerHTML = '';
  if (!memberships?.length) { list.innerHTML = '<div class="empty-state">No groups yet.<br>Tap <strong>+ New</strong> to create one.</div>'; return; }
  const { data: groups } = await supabase.from('groups').select('*').in('id', memberships.map(m=>m.group_id)).order('created_at',{ascending:false});
  groups?.forEach(g => {
    const initial = g.name.charAt(0).toUpperCase();
    const div = document.createElement('div');
    div.className = 'chat-item';
    div.onclick = () => openChat({ id: g.id, name: g.name }, 'group');
    div.innerHTML = `
      <div class="chat-avatar"><div class="avatar-fallback grad-${charGrad(initial)}">${initial}</div></div>
      <div class="chat-info"><div class="chat-name">${escapeHtml(g.name)}</div><div class="chat-preview">Group chat</div></div>
...
async function renderStatusList() {
  ...
  list.innerHTML = '';
  const { data: statuses } = await supabase.from('statuses').select('*, profiles(name)').gt('expires_at', new Date().toISOString()).neq('user_id', currentUser?.id).order('created_at',{ascending:false});
  if (!statuses?.length) { list.innerHTML = '<div class="empty-state">No recent status updates.</div>'; return; }
  statuses.forEach(s => {
    ...
    div.innerHTML = `
      ...
      <div style="flex:1;min-width:0;"><div class="status-name">${escapeHtml(name)}</div><div class="status-time">${escapeHtml(s.text)}</div></div>
...
function openChat(data, type) {
  ...
  document.getElementById('chatHeaderStatus').textContent = type === 'group' ? 'Group chat' : 'online';
...
async function renderMessages() {
  ...
  if (currentChatType === 'user') {
    const { data: m1 } = await supabase.from('messages').select('*').eq('sender_id', currentUser.id).eq('recipient_id', currentChat.id).order('created_at',{ascending:true});
    const { data: m2 } = await supabase.from('messages').select('*').eq('sender_id', currentChat.id).eq('recipient_id', currentUser.id).order('created_at',{ascending:true});
    all = [...(m1||[]), ...(m2||[])].sort((a,b) => new Date(a.created_at) - new Date(b.created_at));
  } else {
...
function subscribeToMessages() {
  unsubscribeMessages();
  // NOTE: Enable Realtime on messages table in Supabase Dashboard → Database → Replication
  const channel = supabase
    .channel(`msgs-${currentUser.id}-${currentChat.id}`)
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => {
      const msg = payload.new;
      if (currentChatType === 'user') {
        const mine = (msg.sender_id === currentUser.id && msg.recipient_id === currentChat.id) ||
                     (msg.sender_id === currentChat.id && msg.recipient_id === currentUser.id);
        if (mine) appendMessage(msg, true);
      } else {
        if (msg.group_id === currentChat.id) appendMessage(msg, true);
      }
    }).subscribe();
...
async function sendMessage() {
  ...
  if (error) { showToast('Message failed'); input.value = text; console.error(error.message); }
}
...
async function searchUsername(query) {
  ...
  const { data } = await supabase.from('profiles').select('id,name,username,vibe').ilike('username', `%${query.toLowerCase()}%`).neq('id', currentUser.id).limit(6);
  if (!data?.length) { resultsDiv.innerHTML = `<div class="search-results-empty">No users found for "@${escapeHtml(query)}"</div>`; return; }
...
async function saveName() {
  ...
  currentUser.name = val; localStorage.setItem('nexus_user', JSON.stringify(currentUser));
  updateProfileUI(); closeModal('editNameModal'); showToast('Name updated');
}

async function editVibe() {
  const v = prompt('Enter new Vibe Word (max 20 chars):');
  if (!v?.trim()) return;
  const trimmed = v.trim().slice(0, 20);
  await supabase.from('profiles').update({ vibe: trimmed }).eq('id', currentUser.id);
  currentUser.vibe = trimmed; localStorage.setItem('nexus_user', JSON.stringify(currentUser));
...
async function handleAvatarUpload(input) {
  ...
  currentUser.avatar = avatarUrl; localStorage.setItem('nexus_user', JSON.stringify(currentUser));
  const img = document.getElementById('profileAvatar');
...
function startPulseSimulation() {
  ...
  if (!feed.querySelector('.pulse-feed-label')) {
    const label = document.createElement('div'); label.className = 'pulse-feed-label'; label.textContent = 'live reactions from your contacts'; feed.appendChild(label);
  }
  const emojis = ['🔥','💙','😭','⚡','🥶','💀','✨','🫀'];
...
function checkPwStrength(pw) {
  const fill = document.getElementById('pwStrengthFill'), label = document.getElementById('pwStrengthLabel');
  let s = 0;
  if (pw.length >= 8) s++; if (pw.length >= 12) s++; if (/[A-Z]/.test(pw)) s++; if (/[0-9]/.test(pw)) s++; if (/[^A-Za-z0-9]/.test(pw)) s++;
  const l = [{pct:'0%',color:'',label:''},{pct:'20%',color:'#FF4B6E',label:'Weak'},{pct:'40%',color:'#f97316',label:'Fair'},{pct:'65%',color:'#f59e0b',label:'Good'},{pct:'80%',color:'#4ade80',label:'Strong'},{pct:'100%',color:'#3B8BFF',label:'Unbreakable ✦'}][s];
  fill.style.width = l.pct; fill.style.background = l.color; label.textContent = l.label; label.style.color = l.color;
}

function setVibe(btn) {
  document.querySelectorAll('.vibe-chip').forEach(c => c.classList.remove('selected'));
  btn.classList.add('selected'); document.getElementById('signupVibe').value = btn.textContent;
}
...
function showToast(msg) {
  ...
  toastTimer = setTimeout(() => { toast.classList.remove('show'); setTimeout(() => toast.classList.add('hidden'), 300); }, 2400);
}
